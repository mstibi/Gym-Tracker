...
CLAN** ucitajClanoveIzDatoteke(FILE* file, int* brojClanova) {
	...
	*brojClanova = 0;
	...
	while (fscanf(file, "%99s %99s %d", ime, prezime, &id) == 3) {
		if (!validirajImePrezime(ime) || !validirajImePrezime(prezime) || id <= 0) {
			greska = 1;
			break;
		}
	...
	if (greska) {
		printf("Otkrivena greska u datoteci. Brisem datoteku.\n");
		fclose(file);
		oslobodiClanove(clanovi, *brojClanova);

		// Proslijedi NULL jer nemaš pokazivač na clanovi iz main-a
		CLAN** temp = NULL;
		int temp_broj = 0;
		obrisiDatoteku(&temp, &temp_broj);
...
void obrisiDatoteku(CLAN*** clanovi, int* brojClanova) {
	const char* naziv = dohvatiTrenutnuDatoteku();

	// Prvo oslobodi memoriju postojećih članova
	if (*clanovi) {
		oslobodiClanove(*clanovi, *brojClanova);
		*clanovi = NULL;
		*brojClanova = 0;
	...
	// Zatim obriši datoteku
	if (remove(naziv) == 0) {
		printf("Datoteka %s je uspjesno obrisana.\n", naziv);
		postaviTrenutnuDatoteku("gym.txt");
		spremiKonfiguraciju();

		// Kreiraj novu praznu datoteku
...
void preimenujDatoteku(void) {
	...
	const char* stariNaziv = dohvatiTrenutnuDatoteku();
	if (rename(stariNaziv, novoIme) == 0) {
		postaviTrenutnuDatoteku(novoIme);
		printf("Datoteka uspjesno preimenovana u %s\n", novoIme);
	}
	else {
